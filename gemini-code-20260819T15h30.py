...
def synthesize_meteor_signal(skew_deg, wind_speed, ratio_3rd, sample_rate=44100, duration=15.0, carrier_freq=1000.0, snr_db=18.0):
    """
    Synthesizes the 3-phase overdense meteor echo given interactive parameters:
    - skew_deg: Trail spatial inclination (degrees)
    - wind_speed: Peak wind velocity (m/s -> Hz offset)
    - ratio_3rd: Relative amplitude of 3rd harmonic relative to fundamental
    """
...
def main():
    ...
    def update_spectrogram(val=None):
        """Recalculates and re-renders the spectrogram whenever sliders move."""
        skew = s_skew.val
        wind = s_wind.val
        ratio = s_ratio.val
    ...
    def play_audio_event(event):
        """Plays the synthesized audio back through system speakers."""
        if HAS_SOUNDDEVICE and current_audio[0] is not None:
            sd.stop()
            sd.play(np.int16(current_audio[0] * 32767), SAMPLE_RATE)
        elif not HAS_SOUNDDEVICE:
            print("Sounddevice library not installed — audio playback unavailable.")
