...
def simulate_physical_meteor_echo_01s_trunk(
    sample_rate=44100,
    duration=60.0,
    ...
    - Phase 1 (0 to 0.1s): Rapid entry phase, forming an unbranched vertical trunk 
      with a narrow initial Doppler spread (±4.5 Hz) lasting from t = 2.0s to 2.1s.
    - Phase 2 (0.1s to 60s): Persistent wind-shear distortion starting at t = 2.1s, 
      bending the trail into approaching (+Hz) and receding (-Hz) specular branches.
    """
    ...
    # Fast formation (~0.01s) and slow electron diffusion decay (35s constant)
    ...
    trunk_duration = 0.10  # Initial trunk lasts exactly 0.1s (100 ms)
    ...
    for i in range(num_samples):
        ...
        if tau < trunk_duration:
            ...
            # Constrained narrow Doppler spread (±4.5 Hz around carrier)
            doppler_z = 4.5 * np.sin(2.0 * np.pi * z)
            specular_weight = np.exp(-(dx0_dz ** 2) / 0.008)

        else:
            # PHASE 2: Wind Shear Distortion Phase (t > 2.1s)
...
def main():
    ...
    print("Generating Physical 2-Phase Meteor Echo Simulation (0.1s Trunk)...")
    t, audio = simulate_physical_meteor_echo_01s_trunk(
    ...
    # 1. Save WAV Audio
    wav_file = "physical_meteor_echo_01s_trunk.wav"
    ...
    plt.title("BRAMS Network — Physical Echo (0.1s Initial Trunk → Persistent Wind Shear)", color='white', fontsize=11)
    ...
    img_filename = "physical_meteor_01s_trunk_spectrogram.png"
