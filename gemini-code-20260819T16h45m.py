...
def synthesize_meteor_signal(skew_deg, wind_speed, ratio_3rd, ratio_5th, sample_rate=44100, duration=15.0, carrier_freq=1000.0, snr_db=18.0):
    ...
    for i in range(num_samples):
        ...
        # Original weight_ping formulation restored
...
def main():
    ...
    ax_r3rd  = fig.add_axes([0.22, 0.12, 0.55, 0.025], facecolor='#1f1f1f')
    ax_r5th  = fig.add_axes([0.22, 0.06, 0.55, 0.025], facecolor='#1f1f1f')

    # Play Button Axis (Positioned cleanly on the right side)
    ...
    def update_spectrogram(val=None):
        ...
        ax_spec.set_title(
            f"BRAMS Meteor Echo  |  Skew: {skew:.1f}°  |  Wind: {wind:.0f} m/s  |  3rd Ratio: {r3rd:.2f}  |  5th Ratio: {r5th:.2f}",
    ...
    s_r3rd.on_changed(update_spectrogram)
    s_r5th.on_changed(update_spectrogram)
