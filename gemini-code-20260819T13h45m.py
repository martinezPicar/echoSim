...
def simulate_skewed_meteor_echo(
    sample_rate=44100,
    duration=15.0,        
    carrier_freq=1000.0,  # Audio carrier reference
    snr_db=18.0,
    trail_skew_angle_deg=12.0  # Initial spatial tilt angle of the meteor trajectory
):
    """
    Simulates a 2-Phase Overdense Meteor Echo with Initial Spatial Skew:
    - Initial Skew: The meteor enters at a non-perpendicular angle relative to 
      the specular direction, creating an initial gradient offset (dx0_dz != 0).
    - Phase 1 (t = 2.0s to 2.1s): 0.1s initial skewed trunk with ±4.5 Hz Doppler spread.
    - Phase 2 (t > 2.1s): Bending driven by dominant fundamental (sin pi*z) + weak 3rd harmonic.
    ...
    # --- 3. Spatial Trail Geometry with Initial Skew ---
    nz = 4000
    z = np.linspace(-1.2, 1.2, nz)
    dz = z[1] - z[0]

    # Initial spatial skew slope (dx/dz offset from vertical alignment)
    skew_rad = np.radians(trail_skew_angle_deg)
    initial_tilt_slope = np.tan(skew_rad)
    
    # Initial trajectory position and spatial derivative with inclination
    ...
    for i in range(num_samples):
        ...
        if tau < trunk_duration:
            ...
            # Offset baseline Doppler shift introduced by initial entry trajectory inclination
            doppler_z = 4.5 * np.sin(2.0 * np.pi * z) + (15.0 * initial_tilt_slope)
            specular_weight = np.exp(-((dx0_dz - initial_tilt_slope) ** 2) / 0.008)

        else:
            # PHASE 2: Wind Shear Bending on Skewed Trail
            shear_tau = tau - trunk_duration
            shear_factor = 0.22 * (1.0 - np.exp(-shear_tau / 1.5))
            
            # Combine initial spatial skew derivative with evolving wind shear deformation
...
def main():
    ...
    DURATION = 15.0       
    CARRIER_FREQ = 1000.0
    SKEW_ANGLE_DEG = 12.0  # Adjustable inclination angle

    print(f"Generating Echo with {SKEW_ANGLE_DEG}° Initial Spatial Skew...")
    t, audio = simulate_skewed_meteor_echo(
    ...
    # 1. Save WAV Audio
    wav_file = "brams_skewed_meteor_echo.wav"
    ...
    plt.title(f"BRAMS Network — Meteor Echo ({SKEW_ANGLE_DEG}° Initial Trail Skew + Dual Harmonic Shear)", color='white', fontsize=11)
    ...
    img_filename = "brams_skewed_meteor_spectrogram.png"
