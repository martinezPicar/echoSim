...
def main():
    ...
    def update_plots(event=None):
        ...
        ax_geom.set_title(
            ...
            # f"h_spec={hspec_km:.1f} km | β={specular_beta_deg:.1f}°",
            f"$h_{{\\mathrm{{spec}}}}$={hspec_km:.1f} km | $\\beta$={specular_beta_deg:.1f}°",
            #f"$\\mathrm{{h_{{spec}}}}$={hspec_km:.1f} km | $\\beta$={specular_beta_deg:.1f}°",
        ...
        ax_spec.set_title(
            ...
            #f"Spectrogram | $f_0$={freq_mhz:.1f} MHz | $d$={dist_km:.0f} km | h_spec={hspec_km:.0f} km | "
    ...
    def save_spectrogram_event(event):
        ...
        if file_path:
            # Extract bounding box of the spectrogram plot axes
            extent = ax_spec.get_window_extent().transformed(fig.dpi_scale_trans.inverted())
            # Increased resolution setting to 600 DPI for ultra high quality output
            # fig.savefig(file_path, bbox_inches='tight', pad_inches=0.1, dpi=600, facecolor=fig.get_facecolor()) # Full window image saving
            fig.savefig(file_path, bbox_inches=extent.expanded(1.22, 1.25), dpi=600, facecolor=fig.get_facecolor()) # ORIGINAL
    ...
    #     if file_path:
    #         # Save exact axes boundary cleanly without artificial padding/stretching
    #         fig.savefig(
    #             file_path, 
    #             bbox_inches=ax_spec.get_window_extent().transformed(fig.dpi_scale_trans.inverted()), 
    #             dpi=300, 
    #             facecolor=fig.get_facecolor()
    #         )
    #         print(f"Saved PNG to: {file_path}")
