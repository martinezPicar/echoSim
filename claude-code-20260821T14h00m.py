...
def main():
    ...
    # Default azimuth = Dourbes->Humain bearing, so the "Azimuth" slider
    # starts at the value that reproduces the original 2-D model exactly.
    _tx = geodetic_to_ecef(DOURBES_LAT_DEG, DOURBES_LON_DEG, DOURBES_ALT_M)
    _rx = geodetic_to_ecef(HUMAIN_LAT_DEG, HUMAIN_LON_DEG, HUMAIN_ALT_M)
    _east, _north, _up = enu_basis(HUMAIN_LAT_DEG, HUMAIN_LON_DEG)
    _v = _rx - _tx
    _v = _v - np.dot(_v, _up) * _up
    DEFAULT_AZIMUTH_DEG = float(
        np.degrees(np.arctan2(np.dot(_v, _east), np.dot(_v, _north))) % 360.0
    )

    # Initial slider settings
    init_traj  = 12.0
    init_azim  = DEFAULT_AZIMUTH_DEG
    init_alt   = 90.0
    init_wind  = 80.0
    init_fund  = 1.00
    init_r3rd  = 0.10
    ...
    # Slider Control Axes (8 sliders, tightened spacing to fit)
    ax_traj  = fig.add_axes([0.18, 0.340, 0.52, 0.020], facecolor='#1f1f1f')
    ax_azim  = fig.add_axes([0.18, 0.308, 0.52, 0.020], facecolor='#1f1f1f')
    ax_alt   = fig.add_axes([0.18, 0.276, 0.52, 0.020], facecolor='#1f1f1f')
    ax_wind  = fig.add_axes([0.18, 0.244, 0.52, 0.020], facecolor='#1f1f1f')
    ax_fund  = fig.add_axes([0.18, 0.212, 0.52, 0.020], facecolor='#1f1f1f')
    ax_r3rd  = fig.add_axes([0.18, 0.180, 0.52, 0.020], facecolor='#1f1f1f')
    ax_r5th  = fig.add_axes([0.18, 0.148, 0.52, 0.020], facecolor='#1f1f1f')
    ax_alpha = fig.add_axes([0.18, 0.116, 0.52, 0.020], facecolor='#1f1f1f')
    ...
    s_traj  = Slider(ax_traj,  'Trajectory Angle (° from vertical)', 1.0, 45.0,  valinit=init_traj,  valstep=0.5,  valfmt='%.1f°',   color='cyan')
    s_azim  = Slider(ax_azim,  'Azimuth (° from N)',     0.0, 360.0, valinit=init_azim, valstep=1.0,  valfmt='%.0f°',    color='deepskyblue')
    s_alt   = Slider(ax_alt,   'Specular Altitude (km)', 70.0, 110.0, valinit=init_alt,  valstep=0.5,  valfmt='%.1f km',  color='dodgerblue')
    ...
    for s in [s_traj, s_azim, s_alt, s_wind, s_fund, s_r3rd, s_r5th, s_alpha]:
    ...
    def update_plots(event=None):
        ...
        ax_geom.set_title(
            f"Trail Deformation | Az={azimuth:.0f}°, Alt={specalt:.1f} km | "
            f"Specular: z={specular_z_km:.2f} km, β={specular_beta_deg:.1f}°",
        ...
        ax_spec.set_title(
            ...
            f"Traj: {traj:.1f}°  |  Az: {azimuth:.0f}°  |  Wind: {wind:.0f} m/s  |  "
