...
def main():
    ...
    ax_noise    = fig.add_axes([0.72, 0.373, 0.23, 0.045], facecolor='#1f1f1f')
    ...
    chk_noise = CheckButtons(ax_noise, ['Add Background Noise'], [True])

    # Fix checkbox stretching: hide rectangular boundary and draw square marker box
    #for rectangle in chk_noise.rectangles:
    #    rectangle.set_visible(False)

    for line_pair in chk_noise.lines:
    ...
    def update_plots(event=None):
        ...
        # Add epsilon to prevent log10(0) division by zero RuntimeWarning when noise is off
        eps = 1e-12
        safe_audio = audio + eps
