...
def main():
    ...
    # ADDED: Azimuth and Elevation slider axes
    ax_az = fig.add_axes([0.18, 0.37, 0.52, 0.022], facecolor='#1f1f1f')
    ax_el = fig.add_axes([0.18, 0.42, 0.52, 0.022], facecolor='#1f1f1f')
    ...
    # ADDED: Azimuth and Elevation sliders
    s_az = Slider(ax_az, 'Meteor Azimuth (°)', 0.0, 360.0, valinit=init_azimuth, valstep=1.0, valfmt='%.0f°', color='lightblue')
    s_el = Slider(ax_el, 'Meteor Elevation (°)', 0.0, 90.0, valinit=init_elevation, valstep=0.5, valfmt='%.1f°', color='lightgreen')
    ...
    # Apply Styling to Sliders (now s_az and s_el are defined)
